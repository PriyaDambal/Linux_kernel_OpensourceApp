...
from kern import get_kernel_github
...
linux_kernel_selected = st.checkbox('Linux Kernel')
...
if st.button("Fetch Data"):
    if emails_input:
        ...
        for email in email_list:
            ...
            kernel_github_data_df = pd.DataFrame()
            ...
            if linux_kernel_selected:
                kernel_github_data_df = get_kernel_github(email)
                if not kernel_github_data_df.empty:
                    print("Successfully fetched Kernel Github Data")
                else:
                    no_commits_found.append(f"linux_kernel: {email}")
                total_commits += len(kernel_github_data_df)
            ...
            user_data_df = pd.concat([chromium_data_df, android_data_df, gitlab_data_df, github_data_df,
                                      kernel_github_data_df], ignore_index=True)
        ...
        # Check if the consolidated dataframe is not empty
